...
def assemble_new_tree(original_mtp, mask, keep_learnable_params=False):
    """
    ...
    Args:
        ...
        keep_learnable_params (bool, Default = False): If provided a train potential will return the pruned coefficient values too.
    ...
    if keep_learnable_params is True:
        new_mtp["is_trained"] = True
        new_mtp["species_count"] = original_mtp["species_count"]
        new_mtp["scaling"] = original_mtp["scaling"]
        new_mtp["species_coeffs"] = original_mtp["species_coeffs"]
        new_mtp["moment_coeffs"] = np.array(original_mtp["moment_coeffs"])[
            mask
        ].tolist()
